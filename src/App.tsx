...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    ...
    return [{ id: "welcome", sender: "zoya", text: "Jai Bhim Dost!" }];
  });

  // Effect to handle audio for the initial greeting if it's new
  useEffect(() => {
    if (messages.length === 1 && messages[0].id === "welcome" && !isMuted) {
      getZoyaAudio("Jai Bhim Dost!").then(audio => {
            ...
            <button
              onClick={() => {
                if (confirm("Are you sure you want to clear the chat history?")) {
                  setMessages([{ id: "welcome", sender: "zoya", text: "Jai Bhim Dost!" }]);
                  resetZoyaSession();
                }
              }}
